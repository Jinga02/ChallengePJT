...
const SignUp = () => {
  ...
  const onCheckId = (userId) => {
    if (userId.length > 20 || userId.length < 5) {
      ...
      axios
        .post(`https://crithub.shop/api/auth/valid/userId?userId=${userId}`)
        .then((res) => {
          if (
            res.data
              ? Swal.fire({
  ...
  const onCheckNickname = (nickname) => {
    if (nickname.length > 10 || nickname.length < 2) {
      ...
      axios
        .post(
          `https://crithub.shop/api/auth/valid/nickname?nickname=${nickname}`,
        )
        .then((res) => {
          if (
            res.data
  ...
  const onSubmit = useCallback(
    (e) => {
      ...
      if (!mismatchError && checkId && checkNickname) {
        ...
        axios
          .post("https://crithub.shop/api/auth/signup", {
            id,
            password,
            email,
            nickname,
